...
const BASE_URL =
	ENVIRONMENT == "DEV" // eslint-disable-line no-ternary
		? `${HOST}:${PORT}`
		: HOST;
...
const userUpdateParams = Joi.object({
	...
	password: Joi.string().trim()
		...
		.max(50)
		.optional(),
	passwordVerify: Joi.string().trim()
		.min(6)
		.max(50)
		.optional(),
	vhs: Joi.string().guid()
...
const usersUpdate = async (req, res, next) => {
	...
	if (!valid) {
		...
		if (value.vhs) {
			userParams = {};
			await HashValidation.deleteHashValidation(value.userID,
				value.vhs);
		}
		if (passwordsPresent) {
			if (passwordsMatch) {
		...
		if (Object.keys(updateParams).length) {
			...
			User.findOneAndUpdate({ _id: ObjectId(value.userID) },
				updateParams,
				{
					new: true,
				})
