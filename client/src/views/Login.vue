  ...
  methods: {
    ...
    onSubmit(event) {
      ...
      if (validation.errors.length) {
        this.$refs.modal.show(validation.errors.reduce((m) => m.message));
      } else {
        this.login();
      }
    },
    ...
    validateInputEmail(email) {
      ...
      if (!email) {
        errors.push({ body: "Email/Username: is not set!" });
      } else if (
        email.includes("@") &&
        !RegExp(/^[a-zA-Z0-9.!#$%&’*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/).test(email)
      ) {
        errors.push({ body: "Email/Username: is not valid!" });
        errors.push({
