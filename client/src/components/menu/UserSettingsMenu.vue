  ...
  >
    <template #default="{ hide }">
      <div class="d-flex justify-content-between px-3 py-2">
        <h4 id="User Settings" class="flex-item">User Settings</h4>
        <b-button class="btn-close flex-item" aria-label="Close" @click="hide"></b-button>
      </div>
      <div class="px-3 py-2">
        <section>
          <b-form @submit="submitFName">
            ...
            >
              <b-form-input
                id="nuInputFname"
                required
                class="nuUserSettingsInput mb-2"
                v-model="firstname"
              ></b-form-input>
...
export default {
  ...
  methods: {
    ...
    destroyUser() {
      axios
        .delete("/api/users", { data: { userID: this.userID } })
        .then(() => {
          this.$store.dispatch("clearAuth");
          this.$router.go(0);
        })
